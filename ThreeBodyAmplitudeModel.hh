...
public:
    ...
    double intensity(const MandelstamTuple &σs, const int &k_amp = -1, const std::vector<int> refζs = {-1, -1, -1, -1}) const;

    // Calculate individual component intensities
    std::vector<double> component_intensities(const MandelstamTuple &σs, const int &k_amp = -1, const std::vector<int> refζs = {-1, -1, -1, -1}) const;

    // Calculate interference terms
    std::vector<std::vector<double>> interference_terms(const MandelstamTuple &σs, const int &k_amp = -1, const std::vector<int> refζs = {-1, -1, -1, -1}) const;

private:
    // Store chains with their labels and coefficients
    std::vector<std::tuple<std::shared_ptr<DecayChain>, std::string, complex, int>> chains_;
};
